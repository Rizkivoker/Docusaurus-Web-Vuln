...
**Authentication** is the process of confirming that a person or system is who they say they are, typically using methods like passwords, fingerprints, or security tokens.

**Digital Identity** refers to a unique online representation of a user. It doesn't always link back to a real person.

**Identify Proofing** ensures that a user is genuinely who they claim to be, connecting their digital identity to a real individual.

**Session Management** is a proccess by which a server maintains the state of an entity interacting with it. This is required for a server to remember how to react to subsequent requests throughout a transaction. It helps a server remember user interactions during a session, using unique session identifiers to track requests. For best practices, refer to the Session Management Cheat Sheet.
...
Users should ne permitted to use their email address as a username, provided the email is verified during signup. Additionally, they should have the option to choose a username other than an email address.


### Authentication Solution and Sensitive Accounts

- Do **NOT** allow login with sensitive accounts (i.e. accounts that can be used internally within the solution such as to a back-end / middle-ware / DB) to any front-end user interface
- Do **NOT** use the same authentication methods for secure and public access.
