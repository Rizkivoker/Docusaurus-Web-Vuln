...
Input Validation should not be used as the *primary* method of preventing **XSS**, **SQL Injection** and other attacks which are covered in respective [cheat sheets](https://cheatsheetseries.owasp.org/) but can significantly contribute to reducing their impact if implemented properly.
...
### 1. Validate Input on Both Client-Side and Server-Side
- Perform client-side validation for a better user experience.
- Always enforce validation on the server-side as the primary defense.

### 2. Use Whitelisting
- Allow only known and expected inputs, such as specific characters, patterns, or data types.
- Reject any input that does not match the whitelist criteria.

### 3. Set Length Limits
- Define minimum and maximum length constraints for input fields.
- Prevent excessively long inputs that can lead to buffer overflow or denial of service attacks.

### 4. Validate Input Format
- Use regular expressions or predefined patterns to ensure inputs conform to expected formats (e.g., email addresses, phone numbers).
- Reject inputs that deviate from the expected format.

### 5. Remove or Escape Dangerous Characters
- Use escaping techniques for inputs that will be rendered as HTML, SQL, or other contexts.
- Filter out or encode characters like `<`, `>`, `'`, `"`, `;`, and `--`.

### 6. Enforce Strong Data Types
- Validate inputs against specific data types (e.g., integers, dates).
- Use type casting and strict comparisons to avoid unexpected behavior.

### 7. Reject Null Bytes and Non-Printable Characters
- Strip null bytes (`\0`) and other non-printable characters from user input.
- Reject inputs containing these characters when unnecessary.

### 8. Sanitize Inputs Before Processing
- Use trusted libraries or frameworks to sanitize inputs for specific contexts (e.g., database queries, HTML rendering).
- Ensure that sanitization does not remove critical data accidentally.

### 9. Avoid Relying Solely on JavaScript Validation
- Do not assume that client-side validation alone is sufficient, as it can be bypassed.
- Use server-side validation to enforce security rules.

### 10. Limit File Uploads
- Restrict file types and sizes for file upload inputs.
- Validate file contents and extensions before accepting them.

### 11. Test and Monitor Inputs
- Regularly test the application for injection attacks and input-based vulnerabilities.
- Log and monitor inputs for suspicious patterns or abuse.

### 12. Implement Input Validation Libraries
- Use well-tested libraries or frameworks to handle input validation securely.
- Avoid writing custom validation logic unless absolutely necessary.

### 13. Protect Against Injection Attacks
- Combine input validation with parameterized queries to prevent SQL injection.
- Avoid dynamically constructing commands or queries using user input.

### 14. Provide Descriptive Error Messages
- Give users clear feedback about invalid inputs without exposing sensitive system details.
- Avoid leaking validation rules in error messages.

### 15. Regularly Review and Update Validation Rules
- Adapt validation rules as application requirements and threats evolve.
- Ensure validation logic is consistent across the application.
