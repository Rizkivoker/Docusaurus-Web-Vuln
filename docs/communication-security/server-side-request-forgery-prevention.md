...
## Introduction to Server Side Request Forgery Prevention
...
This cheat sheet will focus on the defensive point of view and will not explain how to perform this attack. This [talk](./img/Server_Side_Request_Forgery_Prevention_Cheat_Sheet_Orange_Tsai_Talk.pdf) from the security researcher [Orange Tsai](https://twitter.com/orange_8361) as well as this ![document](./img/Server_Side_Request_Forgery_Prevention_Cheat_Sheet_SSRF_Bible.pdf) provide techniques on how to perform this kind of attack.
...
![SSRF Common Flow](./img/Server_Side_Request_Forgery_Prevention_Cheat_Sheet_SSRF_Common_Flow.png)
...
- If the application is vulnerable to [XML eXternal Entity (XXE) injection](https://portswigger.net/web-security/xxe) then it can be exploited to perform a [SSRF attack](https://portswigger.net/web-security/xxe#exploiting-xxe-to-perform-ssrf-attacks), take a look at the **XXE cheat sheet** to learn how to prevent the exposure to XXE.
...
As [Orange Tsai](https://twitter.com/orange_8361) shows in his ![talk](./img/Server_Side_Request_Forgery_Prevention_Cheat_Sheet_Orange_Tsai_Talk.pdf), depending on the programming language used, parsers can be abused. One possible countermeasure is to apply the **allowlist approach** when input validation is used because, most of the time, the format of the information expected from the user is globally known.
...
- It can be used by an attacker to bind a legit domain name to an internal IP address. See the section `Exploitation tricks > Bypassing restrictions > Input validation > DNS pinning` of this ![document](./img/Server_Side_Request_Forgery_Prevention_Cheat_Sheet_SSRF_Bible.pdf).
...
Unfortunately here, the application is still vulnerable to the `DNS pinning` bypass mentioned in this ![document](./img/Server_Side_Request_Forgery_Prevention_Cheat_Sheet_SSRF_Bible.pdf). Indeed, a DNS resolution will be made when the business code will be executed. To address that issue, the following action must be taken in addition of the validation on the domain name:
...
Do not accept complete URLs from the user because URL are difficult to validate and the parser can be abused depending on the technology used as showcased by the following ![talk](./img/Server_Side_Request_Forgery_Prevention_Cheat_Sheet_Orange_Tsai_Talk.pdf) of [Orange Tsai](https://twitter.com/orange_8361).
...
![Case 1 for Network layer protection about flows that we want to prevent](./img/Server_Side_Request_Forgery_Prevention_Cheat_Sheet_Case1_NetworkLayer_PreventFlow.png)
...
Taking into consideration the same assumption in the following **Server Side Request Forgery Prevention Cheat Sheet** for the following sections.
...
1. The application will receive the IP address or domain name of the *TargetedApplication* and it will apply the first validation on the input data using the libraries/regex mentioned in **Server Side Request Forgery Prevention Cheat Sheet**.
2. The second validation will be applied against the IP address or domain name of the *TargetedApplication* using the following block-list approach:
   ...
   - For domain name:
        ...
        2. To prevent the `DNS pinning` attack described in this ![document](./img/Server_Side_Request_Forgery_Prevention_Cheat_Sheet_SSRF_Bible.pdf) , the application will retrieve all the IP addresses behind the domain name provided (taking records *A* + *AAAA* for IPv4 + IPv6) and it will apply the same verification described in the previous point about IP addresses.
...
Similar to the **Server Side Request Forgery Prevention Cheat Sheet**.
...
Draw.io schema XML code for the "![case 1 for network layer protection about flows that we want to prevent](./img/Server_Side_Request_Forgery_Prevention_Cheat_Sheet_Case1_NetworkLayer_PreventFlow.xml)" schema (printscreen are used to capture PNG image inserted into this cheat sheet).

## Mitigation
Here's are the list of the mitigation to prevent Server Side Request Forgery:

### 1. Validate and Sanitize User Inputs
- Only allow URLs from a whitelist of trusted domains.
- Reject any user input that does not conform to strict validation rules.

### 2. Use Allowlists for Network Access
- Restrict outgoing requests to a predefined list of trusted endpoints.
- Deny access to internal networks or sensitive resources by default.

### 3. Disable Unnecessary URL Schemes
- Restrict supported URL schemes to those required for the application (e.g., `https`, `http`).
- Block uncommon or risky schemes like `file://`, `gopher://`, or `ftp://`.

### 4. Implement Network Segmentation
- Isolate sensitive internal systems and resources to prevent direct access from application servers.
- Use firewalls or security groups to enforce strict access controls.

### 5. Restrict DNS Rebinding Attacks
- Validate resolved IP addresses against the original hostname to prevent rebinding.
- Block private and reserved IP ranges (e.g., `127.0.0.1`, `10.0.0.0/8`, `192.168.0.0/16`).

### 6. Use Security Libraries and Frameworks
- Leverage libraries that help detect and mitigate SSRF vulnerabilities.
- For example, use input validation libraries that restrict network calls.

### 7. Limit Response Information
- Avoid returning full server responses or sensitive metadata to users.
- Mask error messages and stack traces in responses.

### 8. Monitor and Log Requests
- Track all outgoing requests, including their source, destination, and purpose.
- Analyze logs for unusual patterns or requests targeting internal or sensitive systems.

### 9. Regularly Update Dependencies
- Keep third-party libraries and frameworks up to date to patch known SSRF vulnerabilities.
- Remove or replace outdated and unsupported software.

### 10. Perform Security Testing
- Conduct penetration testing and dynamic analysis to identify potential SSRF vulnerabilities.
- Use automated tools to scan for SSRF risks in the application.

### 11. Employ Web Application Firewalls (WAF)
- Configure WAF rules to detect and block SSRF attack patterns.
- Integrate the WAF with logging and alerting systems for better visibility.

### 12. Educate Developers and Review Code
- Train developers to recognize and prevent SSRF during the coding phase.
- Perform regular code reviews to identify unsafe network call practices.
